...
class FamilyTreeSpec
  ...
  private val trees = List(
    FamilyTree.graph(persons, relations),
    FamilyTree.sets(persons, relations)
  )

  trees.foreach { aFamily =>
    val treeType = aFamily match {
      case _: GraphFamilyTree => "graph"
      case _: SetsFamilyTree  => "set"
    ...
    treeType when {
      ...
      "siblings" should {
        "return siblings of a person" in {
          val cases = Table(
            ("person", "siblings"),
            (greg, Set(sally)),
            (sally, Set(greg)),
        ...
        "be empty if the person has none" in {
          val cases = Table(
            "person",
            joe,
            jane,
      ...
      "partner" should {
        "return the partner if a person is married" in {
        ...
        "return none if the person isn't married" in {
          val cases = Table(
            "person",
            charles,
            jimmy,
      ...
      "longestMarriage" should {
        "return who are married the longest and how long" in {
          aFamily.longestMarriage shouldBe Marriage(joe, jane, 35)
        }
      }

      "parentsOf" should {
        "return parents of a person" in {
          val cases = Table(
            ("person", "parents"),
        ...
        "return nothing if parents are unknown" in {
          aFamily.parentsOf(joe) shouldBe Set.empty
        }
      }

      "ancestors" should {
        "return who came before a before a person" ignore {
          val cases = Table(
            ("person", "ancestors"),
      ...
      "relation" should {
        "return how two persons are related" ignore {
        ...
        "be empty if there is no relation between two persons" ignore {
   ...
   */
  private lazy val persons: Set[Person] = Set(
    joe,
    jane,
    greg,
    kristen,
  ...
  private lazy val relations: Set[Relation] = Set(
    Sibling(greg, sally),
    Sibling(mike, jimmy),
